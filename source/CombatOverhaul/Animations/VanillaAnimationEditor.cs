...
public sealed partial class DebugWindowManager
{
    ...
    private void CreateVanillaAnimation(VanillaAnimationDocument shapeDocument)
    {
        ...
        VanillaAnimation[] existing = shapeDocument.Shape.Animations ?? [];
        int index = existing.Length;
        shapeDocument.Shape.Animations = existing.Append(animation).ToArray();
    ...
    private void DrawVanillaCenterPanel(VanillaBrowserRow? row, float deltaSeconds)
    {
        ...
        DrawVanillaViewport(row, scene, new NVector2(centerAvailable.X, Math.Max(260f, centerAvailable.Y)));
    }
    ...
    private void DrawVanillaViewport(VanillaBrowserRow row, VanillaAnimationPreviewScene scene, NVector2 requestedSize)
    {
        ...
        renderer.SetGizmoOverlay(null);

        if (effectiveMode == VanillaPreviewMode.Orbit)
        {
            DrawVanillaViewportGizmo(row, scene, renderer, ImGui.GetForegroundDrawList(), min, max, hovered);
        }
        else
        {
    ...
    private void DrawVanillaViewportGizmo(VanillaBrowserRow row, VanillaAnimationPreviewScene scene, VanillaAnimationViewport3DRenderer renderer, ImDrawListPtr drawList, NVector2 min, NVector2 max, bool hovered)
    {
        ...
        renderer.SetGizmoOverlay(new VanillaGizmoOverlay(projection, GizmoMode, hoveredAxis, _vanillaViewportGizmoDragAxis));
    }
    ...
    {
        drawList.AddLine(center, center + axis, color, 2.4f);
    }
    ...
    private static void DrawVanillaViewportGizmoRing(ImDrawListPtr drawList, NVector2[] points, uint color)
    {
        ...
        {
            drawList.AddLine(points[i - 1], points[i], color, 2.4f);
        }
    }
    ...
    private void DrawVanillaElementEditor(VanillaBrowserRow row, VanillaAnimationDocument document, AnimationKeyFrame keyFrame)
    {
        ...
        changed |= DrawNullableDouble("Rotation X", ref element.RotationX);
        changed |= DrawNullableDouble("Rotation Y", ref element.RotationY);
        changed |= DrawNullableDouble("Rotation Z", ref element.RotationZ);
    ...
    private sealed record VanillaEntityOption(EntityProperties EntityType, string Label);
    ...
    private sealed class VanillaAnimationIndexService
    {
        ...
        public void EnsureEntityList(ICoreClientAPI api)
        {
            ...
            foreach (EntityProperties entityType in api.World.EntityTypes ?? [])
            {
                ...
                _entityOptions.Add(new(entityType, code));
            }
        ...
        private void IndexSelectedEntity(ICoreClientAPI api, EntityProperties entityType)
        {
            try
            {
                ...
                if (shape?.Animations != null && shape.Animations.Length > 0)
                {
                    ...
                    {
                        VanillaAnimation animation = shape.Animations[index];
    ...
    private sealed class VanillaAnimationPreviewScene : IDisposable
    {
        ...
        public static VanillaAnimationPreviewScene Create(ICoreClientAPI api, VanillaBrowserRow row)
        {
            ...
            ClientAnimator animator = new(() => 1, shape.Animations, shape.Elements, shape.JointsById, null, null);
        ...
        public void ReloadAnimator(VanillaBrowserRow row)
        {
            ...
            _activeAnimationCode = GetAnimationCode(animation, metadata);
            _metadata.Animation = _activeAnimationCode;
            _animator = new ClientAnimator(() => 1, shape.Animations, shape.Elements, shape.JointsById, null, null);
        ...
        private static VanillaPreviewMeshSet BuildPreviewMeshes(ICoreClientAPI api, VanillaBrowserRow row, Shape shape, ClientAnimator animator, out int textureId)
        {
            ...
            textureId = mesh.TextureIds.Length > 0
                ? mesh.TextureIds[0]
                : api.EntityTextureAtlas.AtlasTextures[0].TextureId;
            ...
            return new(orbit, firstPerson, immersiveFirstPerson);
        }
        ...
        private static MultiTextureMeshRef? TryBuildPlayerFirstPersonMesh(ICoreClientAPI api, MeshData mesh, ClientAnimator animator, bool immersive)
        {
            ...
            MeshData filtered = mesh.EmptyClone();
    ...
    private sealed class VanillaEntityTextureSource : ITexPositionSource
    {
        ...
        public TextureAtlasPosition this[string textureCode]
        {
            get
            {
                ...
                if (_textures.TryGetValue(textureCode, out CompositeTexture? texture))
                {
                ...
                if (_textures.TryGetValue("all", out CompositeTexture? fallbackTexture))
                {
    ...
    private readonly record struct VanillaGizmoOverlay(
        VanillaGizmoProjection Projection,
        TransformGizmoMode Mode,
        TransformGizmoAxis HoveredAxis,
        TransformGizmoAxis DraggedAxis);
    ...
    private sealed class VanillaAnimationViewport3DRenderer : IRenderer
    {
        private readonly ICoreClientAPI _api;
        private FrameBufferRef? _frameBuffer;
        private VanillaAnimationPreviewScene? _scene;
        ...
        private float _panX;
        private float _panY;
        private VanillaPreviewMode _mode = VanillaPreviewMode.Orbit;
        private bool _worldLighting;
        private VanillaGizmoOverlay? _gizmoOverlay;
        private long _updatedAtMs;

        public VanillaAnimationViewport3DRenderer(ICoreClientAPI api)
        {
            _api = api;
            api.Event.RegisterRenderer(this, EnumRenderStage.Ortho, "overhauldevtools-vanilla-animation-preview");
        }

        public double RenderOrder => 1.09;
        public int RenderRange => 9999;

        public void SetVisible(bool visible)
        {
            _visible = visible;
            if (!visible) _gizmoOverlay = null;
        }

        public void SetGizmoOverlay(VanillaGizmoOverlay? overlay)
        {
            _gizmoOverlay = overlay;
        }

        public void SetViewport(VanillaAnimationPreviewScene scene, float x, float y, float width, float height, float yaw, float pitch, float zoom, float panX, float panY, VanillaPreviewMode mode, bool worldLighting)
        {
            _scene = scene;
            _visible = true;
            _x = x;
            ...
            _yaw = yaw;
            _pitch = pitch;
            _zoom = Math.Clamp(zoom, 0.25f, 3.0f);
            _panX = panX;
            _panY = panY;
            _mode = mode;
        ...
        public void OnRenderFrame(float deltaTime, EnumRenderStage stage)
        {
            ...
            if (!_visible || _scene == null) return;
            if (_width <= 32 || _height <= 32) return;
            if (_api.World.ElapsedMilliseconds - _updatedAtMs > 500) return;
            MultiTextureMeshRef meshRef = _scene.GetMeshRef(_mode);
            if (meshRef.Disposed || !meshRef.Initialized) return;

            int framebufferWidth = Math.Max(1, (int)Math.Ceiling(_width));
            int framebufferHeight = Math.Max(1, (int)Math.Ceiling(_height));
            FrameBufferRef frameBuffer = EnsureFrameBuffer(framebufferWidth, framebufferHeight);
            VanillaPreviewCameraState camera = BuildVanillaPreviewCamera(_scene, framebufferWidth, framebufferHeight, _yaw, _pitch, _zoom, _panX, _panY, _mode);
            ...
            try
            {
                ...
                bool classicFirstPerson = _mode == VanillaPreviewMode.FirstPerson && fpHands?.fpModeHandShader != null;
                ...
                SetUniform(shader, "rgbaAmbientIn", _worldLighting ? render.AmbientColor : new Vec3f(1f, 1f, 1f));
                SetUniform(shader, "rgbaLightIn", GetPreviewLight(render));
                SetUniform(shader, "rgbaFogIn", _worldLighting ? render.FogColor : new Vec4f(0f, 0f, 0f, 0f));
                SetUniform(shader, "fogMinIn", _worldLighting ? render.FogMin : 0f);
                SetUniform(shader, "fogDensityIn", _worldLighting ? render.FogDensity : 0f);
                SetUniform(shader, "renderColor", ColorUtil.WhiteArgbVec);
                SetUniform(shader, "alphaTest", 0.01f);
                SetUniform(shader, "depthOffset", GetFirstPersonDepthOffset(fpHands));
                SetUniform(shader, "lightPosition", render.ShaderUniforms.LightPosition3D);
                ...
                SetUniform(shader, "waterWaveCounter", render.ShaderUniforms.WaterWaveCounter);
                SetUniform(shader, "glitchEffectStrength", 0f);
                if (shader.UBOs.TryGetValue("Animation", out UBORef animationUbo))
                {
                    animationUbo.Update(_scene.Animator.Matrices, 0, _scene.Animator.MaxJointId * 16 * 4);
                }
                ...
                render.Render2DTexture(frameBuffer.ColorTextureIds[0], _x, _y + _height, _width, -_height, 204f);
                DrawGizmoOverlay(render);
            }
            finally
            {
                shader?.Stop();
        ...
        private Vec4f GetPreviewLight(IRenderAPI render)
        {
            if (!_worldLighting)
            {
        ...
        private float GetFirstPersonDepthOffset(ModSystemFpHands? fpHands)
        {
            if (_mode != VanillaPreviewMode.FirstPerson) return 0f;
        ...
        private FrameBufferRef EnsureFrameBuffer(int width, int height)
        {
            ...
            DestroyFrameBuffer();
            FramebufferAttrs attrs = new("overhauldevtools-vanilla-preview", width, height)
            {
                Attachments =
                [
        ...
        private void DrawGizmoOverlay(IRenderAPI render)
        {
            ...
            try
            {
                ...
                VanillaGizmoOverlay overlay = _gizmoOverlay.Value;
                VanillaGizmoProjection projection = overlay.Projection;
                int red = ColorUtil.ColorFromRgba(242, 46, 36, 255);
                int green = ColorUtil.ColorFromRgba(51, 214, 71, 255);
                int blue = ColorUtil.ColorFromRgba(56, 122, 255, 255);
                int yellow = ColorUtil.ColorFromRgba(255, 239, 96, 255);
                int white = ColorUtil.ColorFromRgba(255, 246, 204, 255);
                int bounds = ColorUtil.ColorFromRgba(255, 210, 92, 150);
                int helper = ColorUtil.ColorFromRgba(255, 246, 204, 115);

                DrawOverlayBounds(render, projection, bounds, helper);
                DrawOverlayDot(render, projection.Center, 7f, white);

                int xColor = AxisColor(TransformGizmoAxis.X, overlay, red, yellow);
                int yColor = AxisColor(TransformGizmoAxis.Y, overlay, green, yellow);
                int zColor = AxisColor(TransformGizmoAxis.Z, overlay, blue, yellow);

                if (overlay.Mode == TransformGizmoMode.Rotate)
                {
                    DrawOverlayPolyline(render, projection.RingX, xColor, 3.3f);
                    DrawOverlayPolyline(render, projection.RingY, yColor, 3.3f);
                    DrawOverlayPolyline(render, projection.RingZ, zColor, 3.3f);
                    return;
                }

                DrawOverlayAxis(render, projection.Center, projection.AxisX, xColor);
                DrawOverlayAxis(render, projection.Center, projection.AxisY, yColor);
                DrawOverlayAxis(render, projection.Center, projection.AxisZ, zColor);

                if (overlay.Mode == TransformGizmoMode.Scale)
                {
                    DrawOverlayBox(render, projection.Center + projection.AxisX, 6f, xColor);
                    DrawOverlayBox(render, projection.Center + projection.AxisY, 6f, yColor);
                    DrawOverlayBox(render, projection.Center + projection.AxisZ, 6f, zColor);
                }
                else
                {
                    DrawOverlayArrow(render, projection.Center, projection.AxisX, xColor);
                    DrawOverlayArrow(render, projection.Center, projection.AxisY, yColor);
                    DrawOverlayArrow(render, projection.Center, projection.AxisZ, zColor);
                }
            }
            finally
            {
        ...
        private static int AxisColor(TransformGizmoAxis axis, VanillaGizmoOverlay overlay, int normal, int highlighted)
        {
            return axis == overlay.HoveredAxis || axis == overlay.DraggedAxis ? highlighted : normal;
        }

        private static void DrawOverlayBounds(IRenderAPI render, VanillaGizmoProjection projection, int boundsColor, int helperColor)
        {
            if (projection.BoundsCorners.Length >= 8)
            {
                NVector2[] points = projection.BoundsCorners;
                DrawOverlayLine(render, points[0], points[1], boundsColor, 2f);
                DrawOverlayLine(render, points[1], points[2], boundsColor, 2f);
                DrawOverlayLine(render, points[2], points[3], boundsColor, 2f);
                DrawOverlayLine(render, points[3], points[0], boundsColor, 2f);
                DrawOverlayLine(render, points[4], points[5], boundsColor, 2f);
                DrawOverlayLine(render, points[5], points[6], boundsColor, 2f);
                DrawOverlayLine(render, points[6], points[7], boundsColor, 2f);
                DrawOverlayLine(render, points[7], points[4], boundsColor, 2f);
                DrawOverlayLine(render, points[0], points[4], boundsColor, 2f);
                DrawOverlayLine(render, points[1], points[5], boundsColor, 2f);
                DrawOverlayLine(render, points[2], points[6], boundsColor, 2f);
                DrawOverlayLine(render, points[3], points[7], boundsColor, 2f);
            }

            if (projection.HasVisualCenter && (projection.VisualCenter - projection.Center).Length() > 12f)
            {
                DrawOverlayLine(render, projection.Center, projection.VisualCenter, helperColor, 2f);
                DrawOverlayDot(render, projection.VisualCenter, 4f, helperColor);
            }
        }

        private static void DrawOverlayAxis(IRenderAPI render, NVector2 center, NVector2 axis, int color)
        {
            DrawOverlayLine(render, center, center + axis, color, 3.2f);
        }

        private static void DrawOverlayArrow(IRenderAPI render, NVector2 center, NVector2 axis, int color)
        {
            if (axis.LengthSquared() < 1f) return;

            NVector2 tip = center + axis;
            NVector2 dir = NormalizeOrDefault(axis, new NVector2(1f, 0f));
            NVector2 normal = new(-dir.Y, dir.X);
            DrawOverlayLine(render, tip, tip - dir * 15f + normal * 6.5f, color, 3f);
            DrawOverlayLine(render, tip, tip - dir * 15f - normal * 6.5f, color, 3f);
        }

        private static void DrawOverlayPolyline(IRenderAPI render, NVector2[] points, int color, float thickness)
        {
            for (int index = 1; index < points.Length; index++)
            {
                DrawOverlayLine(render, points[index - 1], points[index], color, thickness);
            }
        }

        private static void DrawOverlayLine(IRenderAPI render, NVector2 start, NVector2 end, int color, float thickness)
        {
            NVector2 delta = end - start;
            float length = delta.Length();
            if (length < 0.5f) return;

            int steps = Math.Clamp((int)Math.Ceiling(length / Math.Max(1.2f, thickness * 0.55f)), 1, 180);
            float half = Math.Max(1f, thickness * 0.5f);
            for (int index = 0; index <= steps; index++)
            {
                float t = index / (float)steps;
                NVector2 point = start + delta * t;
                render.RenderRectangle(point.X - half, point.Y - half, 230f, half * 2f, half * 2f, color);
            }
        }

        private static void DrawOverlayDot(IRenderAPI render, NVector2 center, float size, int color)
        {
            float half = size * 0.5f;
            render.RenderRectangle(center.X - half, center.Y - half, 231f, size, size, color);
        }

        private static void DrawOverlayBox(IRenderAPI render, NVector2 center, float size, int color)
        {
            float half = size * 0.5f;
            render.RenderRectangle(center.X - half, center.Y - half, 231f, size, size, color);
        }
    ...
    private sealed class VanillaAnimationDocumentSnapshot
    {
        ...
        public void Restore(VanillaAnimationDocument document)
        {
            ...
            for (int index = 0; index < animationCount; index++)
            {
                ...
                if (document.Shape?.Animations != null && entry.Index >= 0 && entry.Index < document.Shape.Animations.Length &&
                    !ReferenceEquals(document.Shape.Animations[entry.Index], entry.Animation))
                {
                    CopyVanillaAnimation(document.Shape.Animations[entry.Index], _animations[index]);
                }
            }
    ...
    private sealed class VanillaAnimationExportService
    {
        ...
        public string Export(VanillaAnimationDocument document, bool overwrite)
        {
            try
            {
                ...
                string exportRoot = Path.Combine(sourceRoot, "_overhauldevtools_exports");
        ...
        private static void WriteManifest(string outputPath, VanillaAnimationDocument document)
        {
            ...
            File.WriteAllText(outputPath + ".overhauldevtools-manifest.json", manifest.ToString(Formatting.Indented));
        }
    }
