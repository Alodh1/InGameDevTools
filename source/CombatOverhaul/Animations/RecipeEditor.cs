...
public sealed partial class DebugWindowManager
{
    ...
    private sealed class RecipeEditorState
    {
        ...
        public void Draw(ICoreClientAPI api)
        {
            EnsureLoaded(api);

            NVector2 available = ImGui.GetContentRegionAvail();
            float spacing = ImGui.GetStyle().ItemSpacing.X;
            float leftWidth = Math.Clamp(available.X * 0.24f, 300f, 430f);
            float rightWidth = Math.Clamp(available.X * 0.30f, 380f, 560f);
            float centerWidth = Math.Max(420f, available.X - leftWidth - rightWidth - spacing * 2f);
        ...
        private static bool EditStack(JObject parent, string property, string label)
        {
        ...
        private static bool EditStackFields(JObject stack, string label)
        {
            bool changed = false;
            ...
            ImGui.SameLine();
            string code = stack["code"]?.ToString() ?? "";
            ImGui.SetNextItemWidth(210);
            if (ImGui.InputText($"Code##{label}", ref code, 240))
            {
                if (string.IsNullOrWhiteSpace(code)) stack.Remove("code");
                else stack["code"] = code.Trim();
                changed = true;
            }
        ...
        private static bool DrawArrayStackEditor(JObject recipe, string property, string label)
        {
        ...
        private void CreateDraft()
        {
            ...
            AddDocument(document);
            RebuildVisibleEntries();
            _selectedIndex = Math.Max(0, _visibleEntries.FindIndex(entry => entry.Document == document));
